                ...
                <div class="absolute inset-0 bg-slate-900/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-4">
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <form method="POST" class="inline">@csrf @method('DELETE')
                            <button type="button" @click="window.dispatchEvent(new CustomEvent('show-delete-confirm', { detail: { title: 'Hapus Foto?', message: 'Yakin ingin menghapus foto ini? Tindakan ini tidak bisa dibatalkan.', action: '{{ route('admin.galleries.delete', $gallery) }}' } }))" class="w-8 h-8 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center shadow-md">
