...
@section('title', $activity->title . ' - ' . ($settings['school_name'] ?? 'Taman Seminari'))
...
<main class="max-w-container-max mx-auto px-margin-mobile md:px-gutter py-stack-lg">
    <header class="mb-section-gap text-center md:text-left fade-in">
        <a href="{{ url('/#activities') }}" class="inline-flex items-center gap-1 text-secondary font-label-md text-label-md hover:gap-2 transition-all mb-4">
            <span class="material-symbols-outlined text-[18px]">arrow_back</span>
            Kembali ke Kegiatan
        </a>
        <span class="inline-block px-4 py-1 rounded-full bg-secondary-fixed text-on-secondary-fixed font-label-md text-label-md mb-4">Kegiatan</span>
        <h1 class="font-headline-lg text-headline-lg-mobile md:text-headline-lg text-primary mb-4">{{ $activity->title }}</h1>
        <p class="font-body-md text-body-md text-on-surface-variant flex items-center gap-2">
            <span class="material-symbols-outlined text-[18px]">calendar_today</span>
            {{ $activity->activity_date ? $activity->activity_date->format('d F Y') : 'Tanggal belum tersedia' }}
        </p>
    </header>
    ...
    <div class="flex flex-col lg:flex-row gap-gutter fade-in">
        ...
        <aside class="w-full lg:w-80 space-y-stack-lg">
            <div class="bg-white border border-primary/10 rounded-xl p-6">
                <h4 class="font-headline-sm text-headline-sm text-primary mb-4">Kegiatan Lainnya</h4>
                <div class="space-y-4">
                    ...
                    <a href="{{ route('activity.show', $item->id) }}" class="flex gap-3 group">
                        ...
                        <div>
                            ...
                            <span class="text-[12px] text-on-surface-variant">{{ $item->activity_date ? $item->activity_date->format('d M Y') : '-' }}</span>
                        </div>
                    </a>
                    @if(!$loop->last)
                    ...
                    @endif
                    @empty
                    <p class="text-on-surface-variant text-sm">Belum ada kegiatan lain.</p>
                    @endforelse
                </div>
            </div>
            <a href="{{ url('/#contact') }}" class="block bg-primary text-on-primary p-6 rounded-xl text-center hover:bg-primary-container transition-all">
                ...
                <p class="font-body-md text-body-md text-on-primary/70">Info lebih lanjut tentang kegiatan.</p>
            </a>
        </aside>
    </div>
</main>
